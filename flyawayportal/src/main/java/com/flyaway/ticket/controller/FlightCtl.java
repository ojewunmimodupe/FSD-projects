...
public class FlightCtl extends BaseCtl {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(OP_SAVE.equalsIgnoreCase(op)){
			...
			FlightBean bean=(FlightBean)populateBean(request);
				try {
					...
					e.printStackTrace();
					ServletUtility.forward(FlyawayView.ERROR_VIEW, request, response);
					return;
		...
		try {
			model.delete(bean);
			ServletUtility.redirect(FlyawayView.FLIGHT_LIST_CTL, request, response);
		} catch (ApplicationException e) {
			ServletUtility.handleException(e, request, response);
			e.printStackTrace();
		}
		...
		ServletUtility.redirect(FlyawayView.FLIGHT_CTL, request, response);
		return;
}
	...
	protected String getView() {
		...
		return FlyawayView.FLIGHT_VIEW;
	}
